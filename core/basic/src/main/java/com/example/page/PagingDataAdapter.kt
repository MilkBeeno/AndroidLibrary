 ...
 *
 * 2⃣️ 继承 SinglePagingDataAdapter 类型、然后实现 create() 回调创建 ViewHolder，
 *    覆写 onBindViewHolder()、onCreateViewHolder() 等函数。
 *
 * 3⃣️ 直接通过 new 方式创建 SinglePagingDataAdapter 对象并设置各监听回调。
 *
 ...
 * @param clickScope 手势事件监听区域、处理手势的时候需要特别注意的是，如果添加了头部，获取 position 需要减 1
...
open class SinglePagingDataAdapter<T : Any, V : ViewBinding>(
    ...
    private var clickScope: (ViewBindingViewHolder<V>.(SinglePagingDataAdapter<T, V>) -> Unit)? = null,
    ...
    init {
        ...
        addLoadStateListener { combinedLoadStates ->
            val pageSize = if (hasHeader) 1 else 0
            when (combinedLoadStates.source.refresh) {
                ...
                is LoadState.NotLoading -> {
                    if (isRefreshing) {
                        isRefreshing = false
                        refreshedListener?.invoke(if (itemCount <= pageSize) RefreshState.Empty else RefreshState.Success)
                    }
                }

                is LoadState.Error -> {
                    isRefreshing = false
                    refreshedListener?.invoke(if (itemCount < pageSize) RefreshState.Error else RefreshState.Failed)
                }
            }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewBindingViewHolder<V> {
        return create(parent).apply {
            clickScope?.invoke(this, this@SinglePagingDataAdapter)
        }
    }
...
open class MultiplePagingDataAdapter<T : Any>(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        return create(parent, viewType).apply {
            clickScope?.invoke(this, viewType, this@MultiplePagingDataAdapter)
        }
    }
